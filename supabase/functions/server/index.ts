...
async function getAuthenticatedUser(authHeader: string | undefined) {
  if (!authHeader) {
    ...
    return {
      user: null,
      error: { message: "No authorization header", code: 401 },
    };
  }
  ...
  console.log(
    "AUTH: Attempting to authenticate user with token (length:",
    token.length,
    ")",
  );
  ...
  try {
    ...
    if (!user) {
      ...
      return {
        user: null,
        error: { message: "Invalid token - no user found", code: 401 },
      };
    }
  ...
  try {
    ...
    try {
      ...
      const { data: signInData, error: signInError } =
        await supabaseUserClient.auth.signInWithPassword({
          email,
          password,
        });
...
// Path when Supabase forwards URL with function name: /functions/v1/server/signup → path /server/signup
app.post("/server/signup", signupHandler);
...
app.post(`/server/connect-account`, async (c) => {
  try {
    const authHeader = c.req.header("Authorization");
...
// Get connected accounts (register both path variants for Supabase URL forwarding)
const connectedAccountsHandler = async (c: any) => {
  try {
    const authHeader = c.req.header("Authorization");
    if (!authHeader) {
      return c.json({ error: "Unauthorized - Missing token" }, 401);
    ...
    const connectedAccounts =
      (await kv.get(`user:${user.id}:connectedAccounts`)) || [];
...
app.get("/connected-accounts", connectedAccountsHandler);
app.get("/server/connected-accounts", connectedAccountsHandler);
...
app.get(`/server/test-auth`, async (c) => {
  try {
    const authHeader = c.req.header("Authorization");
    ...
    return c.json(
      { error: "Exception during auth test", details: String(err) },
      500,
    );
  }
});
...
app.get(`/server/oauth/instagram/initiate`, async (c) => {
  try {
    const authHeader = c.req.header("Authorization");
    console.log(
      "Instagram OAuth initiation - Auth header present:",
      !!authHeader,
    );
...
app.get(`/server/oauth/instagram/callback`, async (c) => {
  try {
...
app.get(`/server/oauth/facebook/initiate`, async (c) => {
  try {
    const authHeader = c.req.header("Authorization");
    const { user, error } = await getAuthenticatedUser(authHeader);
...
app.get(`/server/oauth/facebook/callback`, async (c) => {
  try {
    const code = c.req.query("code");
    const state = c.req.query("state");
    const error = c.req.query("error");
...
app.get(`/server/oauth/twitter/initiate`, async (c) => {
  try {
    const authHeader = c.req.header("Authorization");
    const { user, error } = await getAuthenticatedUser(authHeader);
    ...
    await kv.set(
      `oauth:state:${state}`,
      { userId: user.id, platform: "twitter", codeVerifier },
      600,
    );
...
app.get(`/server/oauth/twitter/callback`, async (c) => {
  try {
    const code = c.req.query("code");
    const state = c.req.query("state");
    const error = c.req.query("error");
...
app.get(`/server/oauth/linkedin/initiate`, async (c) => {
  try {
    const authHeader = c.req.header("Authorization");
    ...
    const redirectUri = `${supabaseUrl}/functions/v1/server/oauth/linkedin/callback`;
...
app.get(`/server/oauth/linkedin/callback`, async (c) => {
  try {
    const code = c.req.query("code");
    const state = c.req.query("state");
    const error = c.req.query("error");
...
app.get(`/server/oauth/tiktok/initiate`, async (c) => {
  try {
    const authHeader = c.req.header("Authorization");
    const { user, error } = await getAuthenticatedUser(authHeader);
...
app.get(`/server/oauth/tiktok/callback`, async (c) => {
  try {
    const code = c.req.query("code");
    const state = c.req.query("state");
    const error = c.req.query("error");
...
app.get(`/server/oauth/status`, async (c) => {
  try {
    const authHeader = c.req.header("Authorization");
    const { user, error } = await getAuthenticatedUser(authHeader);
...
app.post(`/server/oauth/disconnect`, async (c) => {
  try {
    const authHeader = c.req.header("Authorization");
    const { user, error } = await getAuthenticatedUser(authHeader);
...
app.get(`/analytics/instagram`, async (c) => {
  try {
    const authHeader = c.req.header("Authorization");
    const { user, error } = await getAuthenticatedUser(authHeader);
      ...
      return c.json(
        {
          ...
          details: profileData.error.message,
        },
        400,
      );
    }
    ...
    const engagementRate =
      followers > 0
        ? (totalEngagements / (mediaData.data?.length || 1) / followers) * 100
        : 0;
...
app.get(`/analytics/facebook`, async (c) => {
  try {
    const authHeader = c.req.header("Authorization");
    const { user, error } = await getAuthenticatedUser(authHeader);
    ...
    const followers =
      insightsData.followers_count || insightsData.fan_count || 0;
    const postsCount = postsData.data?.length || 0;
    const engagementRate =
      followers > 0 && postsCount > 0
        ? (totalEngagements / postsCount / followers) * 100
        : 0;
...
app.get(`/analytics/twitter`, async (c) => {
  try {
    const authHeader = c.req.header("Authorization");
    const { user, error } = await getAuthenticatedUser(authHeader);
      ...
      return c.json(
        {
          ...
          details: userData.errors[0]?.message,
        },
        400,
      );
    }
    ...
    const engagementRate =
      followers > 0 && tweetsData.data?.length > 0
...
app.get(`/analytics/linkedin`, async (c) => {
  try {
    const authHeader = c.req.header("Authorization");
    const { user, error } = await getAuthenticatedUser(authHeader);
...
app.get(`/analytics/tiktok`, async (c) => {
  try {
    const authHeader = c.req.header("Authorization");
    const { user, error } = await getAuthenticatedUser(authHeader);
...
app.get("/", (c) => c.json({ message: "Function reachable" }));
