...
export function ProfileSettings({ onClose }: ProfileSettingsProps) {
  ...
  const handleUpdateProfile = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-a8139b1c/update-profile`,
        {
          method: 'POST',
          headers: {
  ...
  const handleUpdatePassword = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-a8139b1c/change-password`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
  ...
  const handleConnectAccount = async (accountId: string) => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-a8139b1c/oauth/${accountId}/initiate`,
        {
          headers: {
            'Authorization': `Bearer ${accessToken}`,
          },
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      const response = await fetch(
        `https://${projectId}.supabase.co/functions/v1/make-server-a8139b1c/delete-account`,
        {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${accessToken}`,
